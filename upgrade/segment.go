...
func UpgradeMirror(opt idl.PgOptions) error {
	...
	if err := restorePrimaryPgControlToMirror(opt.GetTargetDataDir(), opt.GetContent()); err != nil {
		return err
	}
...
func syncPrimaryTablespacesToMirrorTablespaces(opt idl.PgOptions) error {
	for _, tablespace := range opt.Tablespaces {
		...
		// This is already done in lnkTablespacesToTemplate
		// rm -rf /tmp/gpdb5tablespace/m1/demoDataDir0/16386/3/GPDB_6_301908232/16404/*
...
func restorePrimaryPgControlToMirror(targetDataDir string, content int32) error {
	path := filepath.Join(utils.GetBackupMirrorDir(content), "global", "pg_control")
	data, err := ioutil.ReadFile(path)
	if err != nil {
		return xerrors.Errorf("read backup of target mirror pg_control file: %w", err)
	}

	return utils.AtomicallyWrite(filepath.Join(targetDataDir, "global", "pg_control"), data)
}
